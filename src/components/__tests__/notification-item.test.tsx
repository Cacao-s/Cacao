...
describe("NotificationItem", () => {
  ...
  it("renders request_submitted label", () => {
    render(
      <NotificationItem notification={makeNotification({ event_type: "request_submitted" })} />,
    );
  ...
  it("renders request_rejected label", () => {
    render(
      <NotificationItem notification={makeNotification({ event_type: "request_rejected" })} />,
    );
  ...
  it("renders allowance_disbursed label", () => {
    render(
      <NotificationItem notification={makeNotification({ event_type: "allowance_disbursed" })} />,
    );
    expect(screen.getByText("津貼已發放")).toBeDefined();
  });

  it("renders low_balance label", () => {
    render(<NotificationItem notification={makeNotification({ event_type: "low_balance" })} />);
    expect(screen.getByText("餘額偏低")).toBeDefined();
  });

  it("renders member_joined label", () => {
    render(<NotificationItem notification={makeNotification({ event_type: "member_joined" })} />);
    expect(screen.getByText("新成員加入")).toBeDefined();
  });

  it("renders unknown event_type as-is", () => {
    render(<NotificationItem notification={makeNotification({ event_type: "unknown_event" })} />);
  ...
  it("applies font-semibold for unread notifications", () => {
    render(<NotificationItem notification={makeNotification({ is_read: 0 })} />);
    const label = screen.getByText("請款已核准");
    expect(label.className).toContain("font-semibold");
  });

  it("does not apply font-semibold for read notifications", () => {
    render(<NotificationItem notification={makeNotification({ is_read: 1 })} />);
    const label = screen.getByText("請款已核准");
  ...
  it("parses payload with amount_cents", () => {
    ...
    render(<NotificationItem notification={makeNotification({ payload })} />);
  ...
  it("handles invalid JSON payload gracefully", () => {
    render(<NotificationItem notification={makeNotification({ payload: "not json{" })} />);
  ...
  it("calls onClick when clicked", () => {
    ...
    render(<NotificationItem notification={makeNotification()} onClick={handleClick} />);
